...
3. **Path-scoped commit.** Under the mutex, run `git add -- <paths>` (explicit paths only — bundling is still prevented) followed by `git commit --only -m "<msg>" -- <paths>`. The `git add` is required because `--only` rejects untracked/new files; using explicit paths ensures no other implementer's files are staged. `--only` then commits the working-tree content of exactly the named paths, ignoring any other staged content — belt-and-suspenders against bundling even if the index has residue from an interrupted op.
