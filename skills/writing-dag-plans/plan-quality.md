...
**The enumerated patterns in S12, S14 and S15 illustrate the shape. They do not bound it.** Match on *meaning*: if a criterion asserts a negative outcome (S12), makes a universal claim (S14), or asserts a property of the diff (S15), it is in scope even when it matches no listed pattern. The lists are a fast path for the common phrasings, not a ceiling.

This is stated because the alternative was measured. S12's list was repaired by three separate blind fixture runs — passive-voice-only, then `is null` but not `has null`, then `toBeNull` but not `toBe(false)` — and **not one of those runs was aimed at S12.** Each gap was a phrasing that meant exactly what the rule targets and matched nothing in it. The reason is mundane: a fixture written for S12 reaches for the same vocabulary as S12's list, because the same person wrote both, so it lands inside the list and passes. Fixtures written for *other* rules pick their prose for other purposes and walk straight into the gaps. Known live examples still outside the lists: "No new control **appears**" (a negative S12's verbs miss, and a universal claim S14's tokens miss), "the row **is gone**", "**left** empty.

Under-firing is why this matters more than it looks. A missing token produces **no output**, so a rule that silently covers half its domain is indistinguishable from a clean plan. An over-firing rule at least announces itself.

**This widens the triggers only. It does not loosen a single suppressor, nor S15's subject restriction.** That asymmetry is the whole design: the suppressors are already semantic rather than lexical, so they keep holding as the triggers widen — verified against all five fixtures, which still produce their expected verdicts under meaning-matching, `should-pass` included. Widening triggers *and* relaxing suppressors would trade silent under-firing for loud over-firing, which is worse: a rule that fires on the correct form teaches authors to ignore the class.

S13 needs no such note — it is a judgment call by construction, with no list to escape.
